...
int main(int narg, char **args)
{
  ...
  if(single_file_wo_suffix){
message(world, "one single file will be read: " + inputfileName);
  }
  ...
  if(shouldIFindExtrems()){
    ...
    for(int fileId=0; fileId < numberOfFiles;  fileId ++){
      ...
      if(single_file_wo_suffix){
        ...
        fileName = composeFileName(inputfileName,fileId);
      }
      int readings, reminder;
      long long myparticlesToRead = getAndCheckFileNumberOfParticles(world, fileName);
      readings = myparticlesToRead/readLength;
      reminder = myparticlesToRead%readLength;

      std::ifstream theFile( fileName.c_str(), std::ios::binary | std::ios::ate);
  ...
  if(world.myRank == 0){
    ...
    if(flag_1D){
      ...
      for (long long i = 0; i < first_bins; i++){
        ...
        bufstream << fcoord << " " << plotData[i+j*first_bins+k*first_bins*second_bins] << std::endl;
      }
        ...
        for (long long k = 0; k < third_bins; k++){
          for (long long j = 0; j < second_bins; j++){
            for (long long i = 0; i < first_bins; i++){
              ...
              bufstream << fcoord << " " << scoord << " "<< tcoord << " " << plotData[i+j*first_bins+k*first_bins*second_bins] << std::endl;
            }
            bufstream << std::endl;
          }
          bufstream << std::endl;
...
#else
  child.myRank = parent.myRank;
  child.nProc = parent.nProc;
#endif
}
...
int getAndCheckFileNumber(parallelData pdata, std::string strippedFileName){
  ...
  if(fileNumber>pdata.nProc){
    std::stringstream ss;
    ss << "  many files! " << "( " << fileNumber << " > " << pdata.nProc << " )";
    message(pdata, ss.str());
  }
  return fileNumber;
}
